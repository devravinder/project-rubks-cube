...
// Fraction of the panel the mandala fills on desktop.
...
// Guide-ring spacing widened from 5.4 → 6.5 (radii 23.2 / 29.7 / 36.2).
// This pulls the inner ring in and pushes the outer out just enough to grow
// the smallest dot-to-dot gap from 5.72 → ~6.78, so stickers can be drawn
// larger (up to r≈3.39) without overlapping.
const NODES: NodePos[] = [
  { faceletIndex: 0, face: 'U', x: 50, y: 46.75 },
  { faceletIndex: 1, face: 'U', x: 55.79, y: 43.21 },
  { faceletIndex: 2, face: 'U', x: 63, y: 41.44 },

  { faceletIndex: 3, face: 'U', x: 44.21, y: 43.21 },
  { faceletIndex: 4, face: 'U', x: 50, y: 38.85, label: 'U' },
  { faceletIndex: 5, face: 'U', x: 57.21, y: 35.87 },

  { faceletIndex: 6, face: 'U', x: 37, y: 41.44 },
  { faceletIndex: 7, face: 'U', x: 42.79, y: 35.87 },
  { faceletIndex: 8, face: 'U', x: 50, y: 31.56 },

  { faceletIndex: 9, face: 'R', x: 58.01, y: 60.62 },
  { faceletIndex: 10, face: 'R', x: 58.18, y: 67.4 },
  { faceletIndex: 11, face: 'R', x: 56.1, y: 74.53 },

  { faceletIndex: 12, face: 'R', x: 63.97, y: 57.38 },
  { faceletIndex: 13, face: 'R', x: 64.85, y: 64.57, label: 'R' },
  { faceletIndex: 14, face: 'R', x: 63.82, y: 72.31 },

  { faceletIndex: 15, face: 'R', x: 69.1, y: 52.01 },
  { faceletIndex: 16, face: 'R', x: 71.04, y: 59.82 },
  { faceletIndex: 17, face: 'R', x: 71.17, y: 68.22 },

  { faceletIndex: 18, face: 'F', x: 41.99, y: 60.62 },
  { faceletIndex: 19, face: 'F', x: 36.03, y: 57.38 },
  { faceletIndex: 20, face: 'F', x: 30.9, y: 52.01 },

  { faceletIndex: 21, face: 'F', x: 41.82, y: 67.4 },
  { faceletIndex: 22, face: 'F', x: 35.15, y: 64.57, label: 'F' },
  { faceletIndex: 23, face: 'F', x: 28.96, y: 59.82 },

  { faceletIndex: 24, face: 'F', x: 43.9, y: 74.53 },
  { faceletIndex: 25, face: 'F', x: 36.18, y: 72.31 },
  { faceletIndex: 26, face: 'F', x: 28.83, y: 68.22 },

  { faceletIndex: 27, face: 'D', x: 50, y: 82.39 },
  { faceletIndex: 28, face: 'D', x: 44.21, y: 85.93 },
  { faceletIndex: 29, face: 'D', x: 37, y: 87.7 },

  { faceletIndex: 30, face: 'D', x: 55.79, y: 85.93 },
  { faceletIndex: 31, face: 'D', x: 50, y: 90.29, label: 'D' },
  { faceletIndex: 32, face: 'D', x: 42.79, y: 93.27 },

  { faceletIndex: 33, face: 'D', x: 63, y: 87.7 },
  { faceletIndex: 34, face: 'D', x: 57.21, y: 93.27 },
  { faceletIndex: 35, face: 'D', x: 50, y: 97.58 },

  { faceletIndex: 36, face: 'L', x: 27.14, y: 42.8 },
  { faceletIndex: 37, face: 'L', x: 26.97, y: 36.02 },
  { faceletIndex: 38, face: 'L', x: 29.05, y: 28.89 },

  { faceletIndex: 39, face: 'L', x: 21.18, y: 46.04 },
  { faceletIndex: 40, face: 'L', x: 20.3, y: 38.85, label: 'L' },
  { faceletIndex: 41, face: 'L', x: 21.33, y: 31.11 },

  { faceletIndex: 42, face: 'L', x: 16.05, y: 51.41 },
  { faceletIndex: 43, face: 'L', x: 14.11, y: 43.6 },
  { faceletIndex: 44, face: 'L', x: 13.98, y: 35.2 },

  { faceletIndex: 45, face: 'B', x: 72.86, y: 42.8 },
  { faceletIndex: 46, face: 'B', x: 78.82, y: 46.04 },
  { faceletIndex: 47, face: 'B', x: 83.95, y: 51.41 },

  { faceletIndex: 48, face: 'B', x: 73.03, y: 36.02 },
  { faceletIndex: 49, face: 'B', x: 79.7, y: 38.85, label: 'B' },
  { faceletIndex: 50, face: 'B', x: 85.89, y: 43.6 },

  { faceletIndex: 51, face: 'B', x: 70.95, y: 28.89 },
  { faceletIndex: 52, face: 'B', x: 78.67, y: 31.11 },
  { faceletIndex: 53, face: 'B', x: 86.02, y: 35.2 },
]

// Guide circles (3 groups × 3 radii).
const GUIDE_CIRCLES = [
  { cx: 50, cy: 38.85, r: 23.2 },
  { cx: 50, cy: 38.85, r: 29.7 },
  { cx: 50, cy: 38.85, r: 36.2 },

  { cx: 64.85, cy: 64.57, r: 23.2 },
  { cx: 64.85, cy: 64.57, r: 29.7 },
  { cx: 64.85, cy: 64.57, r: 36.2 },

  { cx: 35.15, cy: 64.57, r: 23.2 },
  { cx: 35.15, cy: 64.57, r: 29.7 },
  { cx: 35.15, cy: 64.57, r: 36.2 },
]
 ...
 *
 * Built from the mandala geometry.
 */
type EdgeTrack = {
  cx: number
  cy: number
...
for (const n of NODES) {
  if (n.label) {
    labelOf[n.faceletIndex] = n.label
  }
}
...
function shiftColors(
  ...
  arr.forEach((slot, i) => {
    colors[slot] =
      snapshot[((i - amount) % n + n) % n]
  })
}

/** Apply one move's color permutation. */
function applyMoveToColors(
  move: string,
  colors: Face[],
): Face[] {
  ...
  const faceStep = prime
    ? -FACE_SHIFT
    : FACE_SHIFT

  const innerStep = prime
    ? -INNER_SHIFT
    : INNER_SHIFT
...
function colorsFromHistory(
  history: string[],
): Face[] {
  const colors = HOME_FACE.slice()

  for (const m of history) {
    applyMoveToColors(m, colors)
  }
...
export function AnimatedGraphPanel() {
  ...
  // Drag-to-rotate along a RING track.
  ...
  // Displayed color at each slot.
  const [colors, setColors] = useState<Face[]>(
    () => colorsFromHistory(history),
  )

  // Active animation.
  const [flying, setFlying] =
    useState<FlyingDot[] | null>(null)
  ...
  // Mobile stickers are slightly larger for easier touch interaction.
  const nodeR = isMobile
    ? 3.2
    : NODE_R

  // Invisible touch target.
  const grabR = isMobile
    ? 9
    : 6

  // Keep the current grab radius readable inside the
  // stable pointer listener.
  ...
  // ------------------------------------------------------------
  //
  // The original mobile code used:
  //
  //   fillFactor = 1.0
  //
  // which assumes that the complete drawing fits inside
  // the 0..100 coordinate range.
  //
  // But the actual drawing extends beyond that range:
  //
  // - some stickers extend past y = 100
  // - some guide circles extend past x/y = 100
  //
  // Therefore the outer parts were clipped on mobile.
  //
  // We calculate the REAL bounds of the drawing and fit them
  // into the available Stage with a small safety margin.
  // ------------------------------------------------------------
  ...
  if (isMobile) {
    ...
    // Include sticker radius in the bounds.
    for (const n of NODES) {
      minX = Math.min(
        minX,
        n.x - nodeR,
    ...
    // Include guide circles in the bounds.
    for (const c of GUIDE_CIRCLES) {
      minX = Math.min(
        minX,
        c.cx - c.r,
      )
    ...
    // Extra room for strokes and mobile screen edges.
    const padding = 4
    ...
    // Scale the complete mandala so BOTH dimensions fit.
    scale = Math.min(
      size / contentWidth,
      size / contentHeight,
    )

    // Actual centre of the drawing.
    ...
    // Move the actual drawing centre to the
    // centre of the Stage.
    offsetX =
      size / 2 -
    ...
    offsetY =
      size / 2 -
      contentCenterY * scale
  } else {
    // Keep the existing desktop sizing exactly as before.
    scale =
      (size * FILL_FACTOR) / VIEW
  ...
  // Stage ref — used to attach native pointer listeners
  // on its DOM container.
  const stageRef =
  ...
  useEffect(() => {
    ...
    // Pointer → 100-unit view coordinates.
    //
    // Konva automatically accounts for the Stage transform.
    const toView = () => {
      const p =
    ...
    const onDown = (
      ...
      // Find nearest EDGE-MIDDLE sticker.
      let id = -1
      let bestD = grabRef.current

      for (
        const key of Object.keys(EDGE_TRACK)
      ) {
        const fi = Number(key)
        const p = posOf[fi]
      ...
      if (id < 0) {
        console.log(
          '[drag] DOWN miss (not near an edge-middle sticker)',
          pt,
        )
  ...
  // Keep displayed colors in sync with history.
  useEffect(() => {
    if (!flying) {
      setColors(
        colorsFromHistory(history),
  ...
  // Trigger an arc animation on each single move.
  const prevLen =
    useRef(history.length)
  ...
  useEffect(() => {
    ...
    const buildDots = (
      ...
      return arr.map(
        (slot, i) => {
          // The color currently at srcSlot
          // travels to slot.
          const srcSlot =
            arr[
              (
    ...
    // Hide underlying slots while
    // the dots are being animated.
    const hideSet =
      new Set<number>()
